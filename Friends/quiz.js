...
var quizMaster = (function () {
	...
	function nextHandler(e) {
		...
		if(status.question >= 0) {
			...
			if(checked.length === 0) {
				...
				window.alert("Responde por favor");
				return;
			} else {
	...
	function displayQuiz(successCb) {
		...
		if(current.state === "introduction") {
			html = "<h2>Ultimates</h2><p>" + current.introduction + "</p>" + nextButton();
			...
			html = "<h2>Completado!</h2><p>Has acabado el cuestionario. Tienes "+current.correct+" respuestas correctas de "+data.questions.length+".</p>";
			displayDom.html(html).trigger('create');
			removeUserStatus();
			successCb(current);
		}
